<template>
  <view class="apply">
    <uni-forms ref="form" :model="formData" :label-width="100" :rules="rules">
      <view class="option">
      ...
      <view class="option">
        <uni-forms-item name="age" label="年龄">
          <picker @change="chooseAge" :value="ageIndex" :range="ageList">
            <view class="choose">
              {{ ageList[ageIndex] }}
      ...
      <view class="option">
        <uni-forms-item name="house" label="住房情况">
          <picker @change="chooseHouse" :value="houseIndex" :range="houseList">
            <view class="choose">
      ...
      <view class="option">
        <uni-forms-item name="work" label="职业">
          <input type="text" placeholder="请输入" v-model="formData.work" />
        </uni-forms-item>
      </view>
      <view class="option">
      ...
      <view class="option">
        <uni-forms-item name="phone" label="手机号">
          <input type="text" placeholder="请输入" v-model="formData.phone" />
        </uni-forms-item>
      </view>
      <view class="option">
        <uni-forms-item name="wxCode" label="微信号">
          <input type="text" placeholder="请输入" v-model="formData.wxCode" />
        </uni-forms-item>
      </view>
      ...
      <view class="confirm-btn">
        <button class="confirm-issue" @click="sumbitApply">提交申请</button>
      </view>
    </uni-forms>
  </view>
</template>
...
onLoad(async (option) => {
  ...
  await db
    .collection("apply")
    .where(
      `adopt_id=='${formData.adoptId}'&&user_id=='${userId}'&&state=='申请中'`
    )
    .get()
    .then((res) => {
    ...
    .then(() => {
      if (ifApplied.value) {
        db.collection("apply")
          .where(`adopt_id=='${formData.adoptId}'&&user_id=='${userId}'`)
          .get({ getone: true })
          .then((res) => {
