<template>
  ...
  >
    ...
    <view v-else>
      <view class="receive" v-for="receive in data" :key="receive?._id._value">
        <view
          class="apply"
        ...
        >
            ...
            <view class="pet-info">
              <view class="pet-info-name">
                ...
                <uni-icons
                  ...
                  size="25"
                  color="#777"
                  @click="cancelAdopt(apply._id, apply.adopt_id)"
                ></uni-icons>
              </view>
          ...
          <view class="proposer">
            ...
            >
              <view class="proposer-info">
                <img :src="data?.avatar_file.url" class="avatar" />
                <view class="nickname">
                  {{ data?.nickname }}
                </view>
                <view class="city">
                  {{ apply?.city[0] }}/{{ apply?.city[1] }}/{{ apply?.city[2] }}
                </view>
              </view>
          ...
          <view class="operate">
            <view class="consult" @click="goChat(apply.user_id)">
              <uni-icons type="chat" size="30" color="white"></uni-icons>
              在线咨询
            </view>
            <view class="proposer-detail" @click="goProposerDetail(apply._id)">
              申请详情
            </view>
            <view
              v-if="state == '申请中'"
