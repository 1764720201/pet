<template>
	...
	<view class="city-select">
		...
		>
			...
			<!-- 当前定位城市 -->
			<view class="hot-title" v-if="activeCity && !serachCity">
				当前定位城市
			</view>
			<view class="hot-city" v-if="activeCity && !serachCity">
				<view class="hot-item" @click="cityTrigger(activeCity)">
...
export default {
	...
	computed: {
		...
		searchDatas() {
			...
			for (let i = 0; i < this.cityData.length; i++) {
				if (
					this.cityData[i][this.formatName].indexOf(
						this.serachCity
					) !== -1
				) {
					searchData.push({
	...
	methods: {
		...
		buildCityindexs() {
			...
			for (let i = 0; i < this.cityData.length; i++) {
				// 获取首字母
				const cityLetter = this.getLetter(
					this.cityData[i][this.formatName]
				).firstletter;
				// 获取当前城市首字母的unicode，用作后续排序
				const unicode = this.getLetter(
					this.cityData[i][this.formatName]
				).unicode;
