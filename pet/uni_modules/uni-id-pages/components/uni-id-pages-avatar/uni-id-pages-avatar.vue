...
export default {
	...
	async mounted() {
		usersTable
			...
			.field('avatar_file,mobile,nickname')
			.get()
			.then(res => {
		...
		// }
		// #ifdef H5
		this.isPC = !['ios', 'android'].includes(
			...
			uni.chooseImage({
				...
				success: async res => {
					...
					let tempFile = res.tempFiles[0],
						avatar_file = {
							// #ifdef H5
							extname: tempFile.name.split('.')[
								tempFile.name.split('.').length - 1
							],
							// #endif
							// #ifndef H5
							extname: tempFile.path.split('.')[
								tempFile.path.split('.').length - 1
							]
							// #endif
						},
